...
export class EwtInstallDialog extends LitElement {
  ...
  protected render() {
    ...
    return html`
      <ewt-dialog open .heading=${heading!} scrimClickAction @closed=${this._handleClose} .hideActions=${hideActions}>
        ${heading && allowClosing ? html` <ewt-icon-button dialogAction="close"> ${closeIcon} </ewt-icon-button> ` : ''}
        ${content!}
      </ewt-dialog>
    `;
  }
  ...
  _renderProgress(label: string | TemplateResult, progress?: number) {
    return html` <ewt-page-progress .label=${label} .progress=${progress}></ewt-page-progress> `;
  }
  ...
  _renderError(label: string): [string, TemplateResult, boolean] {
    ...
    const content = html`
      ...
      <ewt-button slot="primaryAction" dialogAction="ok" label="Close"></ewt-button>
    `;
    ...
    content = html`
      ...
      <div class="dashboard-buttons">
        ...
        ${this._client!.nextUrl === undefined
          ...
          : html`
              <div>
                <a href=${this._client!.nextUrl} class="has-button" target="_blank">
        ...
        ${this._isSameFirmware && this._manifest.funding_url
          ? html`
              <div>
                <a class="button" href=${this._manifest.funding_url} target="_blank">
  ...
  _renderProvision(): [string | undefined, TemplateResult, boolean] {
    ...
    if (this._busy) {
      return [
        heading,
        this._renderProgress(this._ssids === undefined ? 'Scanning for networks' : 'Trying to connect'),
        true
      ];
    }
    ...
    if (!this._provisionForce && this._client!.state === ImprovSerialCurrentState.PROVISIONED) {
      ...
      content = html`
        <ewt-page-message .icon=${OK_ICON} label="Device connected to the network!"></ewt-page-message>
        ${showSetupLinks
          ? html`
              <div class="dashboard-buttons">
                ${this._client!.nextUrl === undefined
      ...
      content = html`
        <div>Enter the credentials of the Wi-Fi network that you want your device to connect to.</div>
        ...
        ${this._ssids !== null
          ? html`
              ...
              >
                ${this._ssids!.map(
                  (info, idx) => html`
                    <ewt-list-item .selected=${this._selectedSsid === idx} value=${idx}> ${info.name} </ewt-list-item>
                  `
                )}
                <ewt-list-item .selected=${this._selectedSsid === -1} value="-1"> Join other… </ewt-list-item>
              </ewt-select>
            `
          : ''}
        ${
          // Show input box if command not supported or "Join Other" selected
          this._selectedSsid === -1 ? html` <ewt-textfield label="Network Name" name="ssid"></ewt-textfield> ` : ''
        }
        <ewt-textfield label="Password" name="password" type="password"></ewt-textfield>
        <ewt-button slot="primaryAction" label="Connect" @click=${this._doProvision}></ewt-button>
        <ewt-button
    ...
    if (!this._installConfirmed && this._isSameVersion) {
      ...
      content = this._renderProgress(
        html`
          ...
          <br />
          This will take ${this._installState.chipFamily === 'ESP8266' ? 'a minute' : '2 minutes'}.<br />
      ...
      content = html`
        <ewt-page-message .icon=${OK_ICON} label="Installation complete!"></ewt-page-message>
        <ewt-button
          slot="primaryAction"
          label="Next"
          @click=${() => {
      ...
      content = html`
        <ewt-page-message .icon=${ERROR_ICON} .label=${this._installState.message}></ewt-page-message>
        <ewt-button
          slot="primaryAction"
          label="Back"
          @click=${async () => {
  ...
  private async _doProvision() {
    ...
    const ssid =
      this._selectedSsid === -1
        ? (this.shadowRoot!.querySelector('ewt-textfield[name=ssid]') as EwtTextfield).value
        : this._ssids![this._selectedSsid].name;
    const password = (this.shadowRoot!.querySelector('ewt-textfield[name=password]') as EwtTextfield).value;
    try {
